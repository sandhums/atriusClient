...
//  atriusClient
//
//  Created by Manjinder Sandhu on 08/10/23.
//
...
class KeychainWrapperForToken {
  func storeGenericTokenFor(
    account: String,
    service: String,
    token: String
  ) throws {
    if token.isEmpty {
      try deleteGenericTokenFor(account: account, service: service)
      return
    }
  ...
  func getGenericTokenFor(account: String, service: String) throws -> String {
    let query: [String: Any] = [
      // 1
      kSecClass as String: kSecClassGenericPassword,
      kSecAttrAccount as String: account,
  ...
  func deleteGenericTokenFor(account: String, service: String) throws {
    // 1
    let query: [String: Any] = [
      kSecClass as String: kSecClassGenericPassword,
      kSecAttrAccount as String: account,
